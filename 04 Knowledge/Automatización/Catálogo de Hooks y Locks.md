...
| **Buscador de sesiones** (`search-sessions.py`) | **Lee DOS fuentes y deduplica por `sessionId`:** (1) la copia **local** de `pre-compact.sh` (`.vault-meta/session-logs/`, o `.repo-meta/` en repos de código) — nuestra y portable, pero solo captura **al compactar**; (2) `~/.claude/projects/<proyecto>/`, donde **el harness guarda todas las sesiones solo**, mismo formato JSONL. La diferencia no es teórica: medida en el vault de origen, **2 sesiones la local contra 50 la del harness**. No se usa solo la del harness porque es una ruta **interna y no documentada**, específica de Claude Code: la local queda de ancla portable, y si la otra desaparece esto sigue andando con menos corpus en vez de romperse. Resuelve la ruta por nombre y, si falla, buscando la carpeta que declare este mismo `cwd` (override: `CLAUDE_PROJECTS_DIR`). Es *grep con estructura*: sabe qué es un turno, de quién y cuándo, e ignora los `tool_result` (volcados de archivos = ruido). **Dedupe por `sessionId`**: dos snapshots de una misma sesión larga son un hilo, no dos. Búsqueda **sin acentos ni mayúsculas** (`revision` encuentra `Revisión`). `--list` = inventario · `--role user` = solo lo que pidió el humano · `--regex` · `--full`. | 🟢 **Implementado y probado.** Escaneo directo en vez de índice invertido: el corpus son pocos MB y mantener un índice costaría más que escanear (si algún día crece a cientos de sesiones, ahí entra SQLite FTS5). Es búsqueda **por palabra**, no semántica. ⚠️ Solo ve sesiones de **esta máquina y este usuario**, y la retención del harness no la controlamos — usá `--list` antes de concluir que algo "no existe". |
