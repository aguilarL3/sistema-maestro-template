---
type: SOP
title: "SOP Revisiones"
tags: [sop, revision, auditoria]
estado: 🟢 Activo
prioridad: 🔥 Alta
fecha_creacion: 2026-06-17
timestamp: 2026-06-26T00:00:00Z
id: "SOP-011"
resource:
---

> [!info] La revisión mantiene vivo el sistema
> Sin revisión, el vault se llena de ruido y duplicados.

# SOP Revisiones

## Qué es
Es el proceso para auditar el vault y mantenerlo sano.

## Para qué sirve
Sirve para:
- detectar notas huérfanas
- detectar duplicados
- detectar MOCs faltantes
- detectar notas demasiado largas
- comprobar si los objetivos siguen vigentes
- revisar si los hábitos acompañan los objetivos

## Flujo CE-RE-BRO
### C — Conectar
- notas aisladas
- links faltantes
- temas que deberían relacionarse

### RE — Reagrupar
- tags duplicados
- nombres inconsistentes
- propiedades repetidas
- estructura redundante

### BRO — Descomponer y optimizar
- notas excesivamente largas
- conceptos mezclados
- archivos que deberían partirse
- estructuras que crecieron sin control y pueden simplificarse

## Cuándo revisar
- semanalmente: revisión ligera
- mensualmente: revisión de patrones
- trimestralmente: revisión de objetivos y previsiones
- semestralmente: revisión de valores y principios

## Qué revisar
- diario
- hábitos
- proyectos
- index
- MOCs
- notas evergreen
- notas modelo mental

## Referencias
- [SOP Maestro](<SOP Maestro.md>)
- [SOP Index](<SOP Index.md>)
- [SOP Diario](<SOP Diario.md>)
- [SOP Evergreen Notes](<SOP Evergreen Notes.md>)

## Antes de revisar, lee
- [CE-RE-BRO](<../04 Knowledge/Sistemas y Metodologías/CE-RE-BRO.md>)
- [Evergreen Notes](<../04 Knowledge/Sistemas y Metodologías/Evergreen Notes.md>)
- [Glosario de términos](<Glosario de términos.md>)
- [SOP Diario](<SOP Diario.md>)

## Qué debe salir de una revisión
- notas conectadas
- duplicados resueltos
- prioridades actualizadas
- objetivos revisados
- hábitos ajustados
- archivos archivados
