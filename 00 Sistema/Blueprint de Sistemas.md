...
>[[SOP Interoperabilidad IA]] | [[SOP Conectores]] | [[SOP Skills]] | [[AGENTS]] | [[Glosario de términos]]
...
Este blueprint es la versión **accionable** del [[SOP Interoperabilidad IA]] (que explica la teoría). Acá está el checklist.
...
Todo sistema, sin importar su tamaño, necesita estas 5 capas (ver [[SOP Interoperabilidad IA]] §4):
...
> **Capacidad vs Automatización:** una *Skill* se ejecuta **a demanda** (respondés a un pedido); un *Hook* se dispara **por evento** del ciclo (convierte una convención en enforcement). Ver [[SOP Hooks y Automatización]].
...
> Ver [[SOP Discovery]] (cómo) y [[Investigación Previa (Discovery)]] (por qué). Saltarse esto fue el error del caso Notion.
...
- [ ] Cada doc clave tiene `ultima_revision` en su frontmatter

### Capa 4 — Arquitectura (si conecta sistemas externos)
- [ ] Cada sistema externo tiene su doc en `Conectores/` ([[SOP Conectores]])
- [ ] Incluye modelo, diccionario, dependencias, deuda y changelog

### Capa 5 — Capacidad (si tiene operaciones repetitivas)
- [ ] Las tareas repetitivas están empaquetadas como Skills ([[SOP Skills]])
- [ ] Cada Skill separa metadatos (frontmatter) de lógica

### Capa 6 — Automatización (si hay convenciones que deben cumplirse siempre)
- [ ] Las reglas que no pueden depender de la memoria del agente son **hooks** ([[SOP Hooks y Automatización]])
- [ ] Cada hook tiene kill-switch, está probado (pipe-test) y documentado en el [[Catálogo de Hooks y Locks]]
...
- [ ] El [[Skill - Mantenimiento Sistema]] cubre el sistema nuevo
...
| Documentá el *porqué*, no el *qué* (la IA ve el schema en vivo) | [[SOP Conectores]] §4 |
| La fecha viva vive en el proceso, no en el documento | [[SOP Interoperabilidad IA]] §8 |
...
|---|---|---|---|
| Vault Sistema Maestro | Knowledge base | 🟩 Activo | [[AGENTS]] |
| Notion | Conector (ejemplo ficticio) | ⬜ Ejemplo — reemplazar por tus sistemas | [[Notion - Arquitectura]] |
...
Si vas a armar un sistema nuevo: leé el checklist (§2) y el orden de construcción (§3). Si querés entender el porqué de cada capa, andá al [[SOP Interoperabilidad IA]].
