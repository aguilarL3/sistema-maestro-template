---
type: SOP
title: "SOP Decisiones"
tags: [sop, decisiones]
estado: 🟢 Activo
prioridad: 🔥 Alta
fecha_creacion: 2026-06-17
generated:
  by: human:{{OWNER}}
  at: 2026-06-26T00:00:00Z
id: "SOP-006"
resource:
---

> [!info] Una decisión sin contexto se pierde
> Documenta por qué decidiste algo y qué esperabas obtener.

# SOP Decisiones

## Qué es
Un método para registrar decisiones importantes.

## Para qué sirve
Sirve para:
- recordar por qué elegiste algo
- evitar arrepentimientos por falta de contexto
- comparar resultados con intención
- aprender de decisiones pasadas

## Qué debe incluir
- contexto
- opciones
- criterios
- decisión tomada
- motivo
- revisión futura

## Qué revisar después
- qué ocurrió realmente
- qué aprendiste
- si repetirías esa decisión
- si el criterio fue correcto

## Referencias
- [[Plantilla Decisiones]]
- [SOP Revisiones](<SOP Revisiones.md>)


## Lecturas relacionadas
- [00 Inicio Rapido](<../00 Inicio Rapido.md>)
- [Glosario de términos](<Glosario de términos.md>)
- [Investigación y auditoría de marcos](<../04 Knowledge/Investigación del Sistema/Investigación y auditoría de marcos.md>)
- [MOC - Investigación del Sistema](<../02 MOCs/MOC - Investigación del Sistema.md>)
