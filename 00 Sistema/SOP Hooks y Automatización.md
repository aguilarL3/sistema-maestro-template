...
- 🔴 **Alcance: una máquina.** Como `.vault-meta/` está gitignoreado, un lock **no viaja al remoto y las demás personas no lo ven**. Es lo correcto —un lock es estado efímero, no historial: versionarlo dejaría locks huérfanos en el árbol de todos— pero hay que decir el límite en voz alta: `wiki-lock.sh` resuelve **agente ↔ agente en un mismo clon**, y **no protege nada entre personas**. En vault compartido el sustituto es la regla del **escritor único sobre los hotspots** ([SOP Git](<SOP Git y Flujo de Trabajo.md>) §11.3), que es convención hablada y no tiene herramienta. Ver [SOP Multi-Agente](<SOP Multi-Agente.md>) §5.1.
